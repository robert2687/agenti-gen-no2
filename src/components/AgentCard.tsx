...
const AgentIcon = ({ name }: { name: string }) => {
  ...
  switch (name) {
    ...
    case 'Coder':
      return <CoderIcon {...iconProps} />;
    case 'Reviewer':
      return <ReviewerIcon {...iconProps} />;
    case 'Deployer':
      return <DeployerIcon {...iconProps} />;
    default:
      return null;
  }
};
...
const AgentCard: React.FC<AgentCardProps> = ({ agent, isSelected, isCurrent, onClick }) => {
  ...
  const selectedClass = isSelected
    ? 'ring-2 ring-offset-2 ring-offset-slate-800 ring-indigo-400'
    : '';
  ...
  return (
    ...
    >
      <div className={`flex-shrink-0 ${styles.text}`}>
        ...
        (isCurrent && agent.status !== AgentStatus.COMPLETED) ? (
