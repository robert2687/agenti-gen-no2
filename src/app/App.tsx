  ...
  const handleStartGeneration = useCallback(async () => {
    ...
    for (let i = 0; i < agentStates.length; i++) {
      ...
      while (needsClarificationLoop) {
        ...
        const streamedOutput = await geminiService.runAgentStream(
          agentStates[i],
          agentInputForRun,
          onChunk
        );
        ...
        if (agentStates[i].name === 'Coder') {
          ...
          if (request && request.ask === 'Architect') {
            ...
            agentStates[i] = {
              ...agentStates[i],
              output: currentCoderOutput.replace(streamedOutput, '') + questionLog,
            };
            setAgents([...agentStates]);
            ...
            onAnswerChunk("\n**Architect's Answer:**\n> ");
            const answer = await geminiService.getClarificationAnswerStream(
              architect,
              request.question,
              architect.output,
              onAnswerChunk
    ...
    <div className="min-h-screen bg-slate-900 font-sans flex flex-col">
      ...
      {error && (
        <div className="fixed bottom-4 right-4 bg-red-800 text-white p-4 rounded-lg shadow-lg">
          ...
          <p>{error}</p>
          <button onClick={() => setError(null)} className="absolute top-1 right-2 text-xl">
            &times;
          </button>
        </div>
      )}
